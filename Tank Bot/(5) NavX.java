...
public class Robot extends IterativeRobot {
  ...
  Compressor c = new Compressor(0); //declares compressor
    ...
    if(timer.get() > 1 && timer.get() < 4)  { //if the timer is in between 1 and 4 seconds
					tank.arcadeDrive(.7, 0); //drives forward
					}
					else if (timer.get() > 4) { //if timer is bigger than 4 seconds
						
						tank.arcadeDrive(0,0); //stops movement
					}
